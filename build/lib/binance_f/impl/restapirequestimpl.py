...
class RestApiRequestImpl(object):
    ...
    def _create_request_by_download_with_signature(self, url, builder):
        request = RestApiRequest()
        request.method = "POST"
        request.host = self.__server_url
    ...
    def download_id(self,symbol,startTime,endTime,dataType):
        builder = UrlParamsBuilder()
        builder.put_url("symbol", symbol)
        builder.put_url("startTime", startTime)
        builder.put_url("endTime", endTime)
        builder.put_url("dataType", dataType)
        request = self._create_request_by_download_with_signature("https://api2.binance.com/sapi/v1/futuresHistDataId", builder)
        def parse(json_wrapper):
